...
class FaceDetector(Protocol):
    def detectMultiScale3(self, image: NDArray[np.uint8], **kwargs: object) -> tuple: ...
...
class OpenCvFaceProcessor:
    ...
    def _face_embedding(
        ...
        normalized = cv2.resize(normalized, self._HOG_SIZE, interpolation=cv2.INTER_AREA)
    ...
    def _feature_hash(self, descriptor: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        signs = np.where(
            ((feature_positions * np.uint64(2_246_822_519)) >> np.uint64(16)) & np.uint64(1),
            1.0,
            -1.0,
        ).astype(np.float32)
